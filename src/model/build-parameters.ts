...
class BuildParameters {
	...
	public useSpotInstances!: boolean;
	...
	public static shouldUseRetainedWorkspaceMode(
	...
	static async create(): Promise<BuildParameters> {
		...
		const androidSdkManagerParameters =
			AndroidVersioning.determineSdkManagerParameters(
		...
		return {
			...
			gitPrivateToken:
				Input.gitPrivateToken ?? (await GithubCliReader.GetGitHubAuthToken()),
			...
			cloudRunnerBranch: CloudRunnerOptions.cloudRunnerBranch
				.split("/")
				.reverse()[0],
			...
			buildGuid: CloudRunnerBuildGuid.generateGuid(
				Input.runNumber,
				Input.targetPlatform,
			),
			...
			maxRetainedWorkspaces: Number.parseInt(
				CloudRunnerOptions.maxRetainedWorkspaces,
			),

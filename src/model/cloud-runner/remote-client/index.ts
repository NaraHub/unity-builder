...
export class RemoteClient {
	@CliFunction(
		`remote-cli-pre-build`,
		`sets up a repository, usually before a game-ci build`,
	)
	...
	public static async remoteClientPostBuild(): Promise<string> {
		...
		try {
			...
			if (fs.existsSync(libraryFolderHost)) {
				const libraryEntries = await fs.promises
					.readdir(libraryFolderHost)
					.catch(() => [] as string[]);
				if (libraryEntries.length > 0) {
					await Caching.PushToCache(
						CloudRunnerFolders.ToLinuxFolder(
							`${CloudRunnerFolders.cacheFolderForCacheKeyFull}/Library`,
						),
						CloudRunnerFolders.ToLinuxFolder(
					...
					RemoteClientLogger.log(
						`Skipping Library cache push (folder is empty)`,
					);
				}
			} else {
			...
			RemoteClientLogger.logWarning(
				`Library cache push skipped with error: ${error.message}`,
			);
		}
		...
		try {
			...
			if (fs.existsSync(buildFolderHost)) {
				const buildEntries = await fs.promises
					.readdir(buildFolderHost)
					.catch(() => [] as string[]);
				if (buildEntries.length > 0) {
					await Caching.PushToCache(
						CloudRunnerFolders.ToLinuxFolder(
							`${CloudRunnerFolders.cacheFolderForCacheKeyFull}/build`,
						),
						CloudRunnerFolders.ToLinuxFolder(
		...
		try {
			...
			for (const file of files) {
				const fileContents = fs.readFileSync(
					path.join(gameCiCustomHooksPath, file),
					`utf8`,
				);
	...
	public static async bootstrapRepository() {
		...
		await Caching.PullFromCache(
			CloudRunnerFolders.ToLinuxFolder(
				CloudRunnerFolders.libraryCacheFolderFull,
			),
			CloudRunnerFolders.ToLinuxFolder(
				CloudRunnerFolders.libraryFolderAbsolute,
			),
	...
	private static async cloneRepoWithoutLFSFiles() {
		...
		if (
			...
			await CloudRunnerSystem.Run(
				`rm -r ${CloudRunnerFolders.repoPathAbsolute}`,
			);
			CloudRunnerLogger.log(
				`${CloudRunnerFolders.repoPathAbsolute} repo exists, but no git folder, cleaning up`,
			);
		}
		...
		if (
			...
			await CloudRunnerSystem.Run(
				`git fetch && git reset --hard ${CloudRunner.buildParameters.gitSha}`,
			);
		...
		RemoteClientLogger.log(
			`Initializing source repository for cloning with caching of LFS files`,
		);
		await CloudRunnerSystem.Run(
		...
		assert(
			fs.existsSync(path.join(`.git`, `lfs`)),
			"LFS folder should not exist before caching",
		);
	...
	static async replaceLargePackageReferencesWithSharedReferences() {
		CloudRunnerLogger.log(
			`Use Shared Pkgs ${CloudRunner.buildParameters.useLargePackages}`,
		);
	...
	private static async pullLatestLFS() {
		...
		try {
			...
			RemoteClientLogger.log(
				`Pulled LFS files without explicit token configuration`,
			);
		...
		try {
			...
			if (gitPrivateToken) {
				RemoteClientLogger.log(
					`Attempting to pull LFS files with GIT_PRIVATE_TOKEN...`,
				);
				await CloudRunnerSystem.Run(
				...
				await CloudRunnerSystem.Run(`git lfs checkout || true`, true);
				RemoteClientLogger.log(
					`Successfully pulled LFS files with GIT_PRIVATE_TOKEN`,
				);
			...
			RemoteClientLogger.logCliError(
				`Failed with GIT_PRIVATE_TOKEN: ${error.message}`,
			);
		}
		...
		try {
			...
			if (githubToken) {
				RemoteClientLogger.log(
					`Attempting to pull LFS files with GITHUB_TOKEN fallback...`,
				);
				await CloudRunnerSystem.Run(
					`git config --global --unset-all url."https://github.com/".insteadOf || true`,
				);
				...
				await CloudRunnerSystem.Run(`git lfs checkout || true`, true);
				RemoteClientLogger.log(
					`Successfully pulled LFS files with GITHUB_TOKEN`,
				);
			...
			RemoteClientLogger.logCliError(
				`Failed with GITHUB_TOKEN: ${error.message}`,
			);
		}

		// If we get here, all strategies failed; continue without failing the build
		RemoteClientLogger.logWarning(
			`Proceeding without LFS files (no tokens or pull failed)`,
		);
	}

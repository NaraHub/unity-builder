...
class AWSTaskRunner {
  ...
  static async runTask(
    ...
    const runParameters = {
      ...
      launchType: CloudRunner.buildParameters.useSpotInstances ? undefined : 'FARGATE',
  ...
  private static async waitUntilTaskRunning(taskArn: string, cluster: string) {
    try {
      ...
      if (exitCode !== undefined && exitCode === 0) {
        CloudRunnerLogger.log(`Task completed successfully (Exit Code 0). Suppressing Waiter error.`);

        return;
      }
      CloudRunnerLogger.log(`Task failed or was interrupted. Propagating error.`);

...
class AWSTaskRunner {
	...
	static async runTask(
		...
		const taskDefinition =
			taskDef.taskDefResources?.find(
				(x) => x.LogicalResourceId === "TaskDefinition",
			)?.PhysicalResourceId || "";
		const SubnetOne =
			taskDef.baseResources?.find(
				(x) => x.LogicalResourceId === "PublicSubnetOne",
			)?.PhysicalResourceId || "";
		const SubnetTwo =
			taskDef.baseResources?.find(
				(x) => x.LogicalResourceId === "PublicSubnetTwo",
			)?.PhysicalResourceId || "";
		...
		const runParameters = {
			...
			capacityProviderStrategy: CloudRunner.buildParameters.useSpotInstances
				? [
		...
		if (
			JSON.stringify(runParameters.overrides.containerOverrides).length > 8192
		) {
		...
		const task = await AwsClientFactory.getECS().send(
			new RunTaskCommand(runParameters as any),
		);
		if (!task.tasks || task.tasks.length === 0) {
			CloudRunnerLogger.log(
				`RunTaskCommand response: ${JSON.stringify(task, undefined, 4)}`,
			);
			throw new Error(
				"No tasks were created. Check the RunTaskCommand response above.",
			);
		}
		...
		CloudRunnerLogger.log(
			`Container State: ${JSON.stringify(containerState, undefined, 4)}`,
		);
		...
		if (
			taskData?.stoppedReason === "Essential container in task exited" &&
			exitCode === 1
		) {
	...
	private static async waitUntilTaskRunning(taskArn: string, cluster: string) {
		try {
			...
			const taskAfterError = await AWSTaskRunner.describeTasks(
				cluster,
				taskArn,
			);
			...
			CloudRunnerLogger.log(`Task exited with code: ${exitCode}`);

			if (exitCode !== undefined && exitCode === 0) {
				CloudRunnerLogger.log(
					`Task completed successfully (Exit Code 0). Suppressing Waiter error.`,
				);

				return;
			}
			CloudRunnerLogger.log(
				`Task failed or was interrupted. Propagating error.`,
			);
	...
	static async streamLogsUntilTaskStops(
		...
		while (shouldReadLogs) {
			...
			({ timestamp, shouldReadLogs } =
				AWSTaskRunner.checkStreamingShouldContinue(
					taskData,
					timestamp,
					shouldReadLogs,
				));
	...
	private static checkStreamingShouldContinue(
		...
		if (taskData?.lastStatus !== "RUNNING") {
			if (timestamp === 0) {
				CloudRunnerLogger.log(
					"## Cloud runner job stopped, streaming end of logs",
				);
				timestamp = Date.now();
			}
			if (timestamp !== 0 && Date.now() - timestamp > 30000) {
				CloudRunnerLogger.log(
					"## Cloud runner status is not RUNNING for 30 seconds, last query for logs",
				);
		...
		if ((records.Records ?? []).length > 0 && iterator) {
			for (const record of records.Records ?? []) {
				const json = JSON.parse(
					zlib
						.gunzipSync(Buffer.from(record.Data as unknown as string, "base64"))
						.toString("utf8"),
				);
